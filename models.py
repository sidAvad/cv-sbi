...
AE pre-training components:
  AutoencoderEncoder                : 24-ch CNN → latent_dim, phase 1
...
class AutoencoderEncoder(nn.Module):
    """24-ch continuous CNN → latent_dim. Used in phase 1 of AE pre-training.

    Expanding channel structure: 24→64→128→256→256.
    """
    ...
    def forward(self, x):
        h = self.cnn(x.view(-1, N_CONT, T)).transpose(1, 2)  # (B, T', 256)
        w = self.attn_pool(h).softmax(dim=1)
        h = (w * h).sum(dim=1)
        return self.proj(h)
...
class ReducedAutoencoderEncoder(nn.Module):
    ...
    proj_hidden: if set, uses two-stage projection feat_dim→proj_hidden→latent_dim
                 instead of a single linear. Use proj_hidden=128 for latent_dim=64.
    """
...
class LipschitzReducedAutoencoderEncoder(nn.Module):
    ...
    is clamped to at most `sn_ceiling`.

    proj_hidden: if set, uses two-stage projection feat_dim→proj_hidden→latent_dim
                 with SN on both layers. Use proj_hidden=128 for latent_dim=64.
...
class WaveformDecoder(nn.Module):
    ...
    n_layers: total number of Linear layers (default 2 for backward compat).
    out_channels: N_CHANNELS (28, old runs) or N_CONT (24, new runs).
    Output shape: (B, out_channels*T) — flat. Reshape in training code as needed.
    """

    def __init__(self, latent_dim=LATENT_DIM, hidden=512, n_layers=2, out_channels=N_CHANNELS):
        super().__init__()
